...
public class RobotContainer {
        ...
        private void configureBindings() {
                ...
                drivetrain.setDefaultCommand(
                                ...
                                drivetrain
                                                .applyRequest(() -> drive.withVelocityX(
                                                                -Math.pow(c_DriverController.getLeftY(), 5) * MaxSpeed) // Drive
                                                                                                                        // forward
                                                                                                                        // with
                                                                                                                        // negative
                                                                                                                        // Y
                                                                                                                        // (forward)
                                                                .withVelocityY(-Math.pow(c_DriverController.getLeftX(),
                                                                                5) * MaxSpeed) // Drive left
                                                                                               // with negative
                                                                                               // X (left)
                                                                .withRotationalRate(-c_DriverController.getRightX()
                ...
                c_DriverController.b().whileTrue(drivetrain.applyRequest(() -> point
                                .withModuleDirection(new Rotation2d(-c_DriverController.getLeftY(),
                                                -c_DriverController.getLeftX()))));

                // Run SysId routines when holding back/start and X/Y.
                // Note that each routine should be run exactly once in a single log.
                c_DriverController.back().and(c_DriverController.y())
                                .whileTrue(drivetrain.sysIdDynamic(Direction.kForward));
                c_DriverController.back().and(c_DriverController.x())
                ...
                c_DriverController.leftBumper().onTrue(PathUtils.allignLeft()).onFalse(new InstantCommand(()->{CommandScheduler.getInstance().cancelAll();}));
                
                c_DriverController.rightBumper().onTrue(PathUtils.allignRight()).onFalse(new InstantCommand(()->{CommandScheduler.getInstance().cancelAll();}));
